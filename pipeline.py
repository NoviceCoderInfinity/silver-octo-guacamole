...
SVG 0.88 baseline: describe → per-style specialists (with frames) → optional selector.

CAPTION_ASSEMBLY controls the single experimental IV on this branch:
  - portfolio_select: best-of-2 candidates + frame-grounded copy-only selector (SVG)
  - single_shot: one caption per style from the specialist; no selector call
"""
import json
import os
import sys
...
def caption_video(video_url: str, styles: list[str], client: CaptionClient) -> dict:
    """Describe → specialists (with frames). Assembly mode from config.CAPTION_ASSEMBLY."""
    frames_b64 = _extract_frames_b64(video_url)
    ...
    assembly = (getattr(config, "CAPTION_ASSEMBLY", "single_shot") or "single_shot").strip().lower()
    print(f"[pipeline] caption_assembly={assembly}", file=sys.stderr)
    ...
    out = {s: result.get(s, "") for s in styles}
    if getattr(config, "ENABLE_OUTPUT_SAFETY", False):
        out = output_safety.clean_captions(out)
    return out
